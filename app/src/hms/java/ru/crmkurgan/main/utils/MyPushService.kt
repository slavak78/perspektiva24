...
class MyPushService : HmsMessageService() {
    ...
    override fun onMessageReceived(remoteMessage: com.huawei.hms.push.RemoteMessage?) {
        ...
        remoteMessage?.let {
            if (it.data.isNotEmpty()) {
                try {
                    val userdata = JSONObject(it.data)
                    val title = userdata.optString(Constants.TITLE)
                    val message = userdata.optString(Constants.BODY)
                    val pic = userdata.optString(Constants.ICON)
                    val all = userdata.optString(Constants.ALL)
                    from = userdata.optString(Constants.SENDER)
                    val id = userdata.optString(Constants.ID)
                    type = userdata.optString(Constants.TYPE)
                    if (all == "1") {
                        ssd(title, message, pic)
                        saveToServerAll(title, message, from, id)
                    } else {
                        val uid1 = userdata.optInt(Constants.UID)
    ...
    private fun sendNotification(
        ...
        val bitmap1 = bitmap?.let { Functions.getCircleBitmap(it) }
